...
"""
This is file which manages data(i.e. read, write) from files like tasks.json, setting.json etc.
...
try:
  from paths import TASKS_FILE, SETTINGS_FILE, WALLPAPER_URLS_JSON, BLACK_LISTED_WINDOW_FILE
  import task_manager
except ImportError:
  from APP.paths import TASKS_FILE, SETTINGS_FILE, WALLPAPER_URLS_JSON, BLACK_LISTED_WINDOW_FILE
  import APP.task_manager as task_manager

try:
  load_dotenv()
  PIDOES_WALLPAPER_API_URL = os.getenv("PIDOES_WALLPAPER_API_URL")
  print(PIDOES_WALLPAPER_API_URL)
except Exception:
...
def fetch_wallpaper_urls():
  if PIDOES_WALLPAPER_API_URL:
    try:
      ...
      with open(WALLPAPER_URLS_JSON, "w") as f:
...
def read_wallpaper(category=None):
  if Path(WALLPAPER_URLS_JSON).exists():
    try:
      with open(WALLPAPER_URLS_JSON, "r") as f:
        data = json.load(f)
      ...
      print(f"read_wallpaper error: {e}")
  return None
...
def write_settings(data):
  ...
  try: 
    ...
    print(f"write_settings error : {e}")
  
def fetch_black_listed_window():
  black_listed_window_file = Path(BLACK_LISTED_WINDOW_FILE)
  try : 
    if black_listed_window_file.exists:
      with open(BLACK_LISTED_WINDOW_FILE, "r") as f:
         black_listed_data = json.load(f)
      return black_listed_data
  except Exception as e :
    print (f"fetch_black_listed_window() error : {e}")
